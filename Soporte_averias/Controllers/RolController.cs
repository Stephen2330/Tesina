...
namespace Soporte_averias.Controllers
{
	...
	public class RolController : Controller
    {
		...
		public ActionResult Index(int? page, string searchText)
		{
			...
			IEnumerable<TBL_Rol> rol;
			rol = db.TBL_Rol.AsQueryable();

			if (!string.IsNullOrEmpty(searchText))
			{

				rol = rol.Where(m => m.TC_Nombre.Contains(searchText));
			}

			int totalItems = rol.Count(); //Cant. elementos totales
			int totalPages = (int)Math.Ceiling((double)totalItems / pageSize); //Cant. total de páginas
			ViewBag.totalPages = totalPages;
			ViewBag.CurrentFilter = searchText;
			var rolesOrdenados = rol.OrderBy(m => m.TC_Nombre);
			var rolesPaginas = rolesOrdenados.Skip((pageNumber - 1) * pageSize).Take(pageSize);

			return View(rolesPaginas);
        ...
        public ActionResult Create([Bind(Include = "TN_IdRol,TC_Nombre,TC_Descripcion")] TBL_Rol tBL_Rol)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "TN_IdRol,TC_Nombre,TC_Descripcion")] TBL_Rol tBL_Rol)
        {
            if (ModelState.IsValid)
            {
		...
		public ActionResult ExportToPdf(string searchText, int? page)
		{
			...
			var actividad = db.TBL_Rol.AsQueryable();
			...
			memoryStream.Close();
			return File(bytes, "application/pdf", "Datos_roles.pdf");
		}
		...
		public ActionResult ExportToExcel(string searchText)
		{
			...
			using (var package = new ExcelPackage())
			{
				...
				return File(excelBytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", "Datos_roles.xlsx");
			}
		}
