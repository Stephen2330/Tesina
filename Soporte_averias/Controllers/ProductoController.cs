...
namespace Soporte_averias.Controllers
{
	...
	public class ProductoController : Controller
    {
		...
		public ActionResult Create([Bind(Include = "TN_IdProducto,TC_Nombre,TC_Descripcion,TN_Cantidad_Inventario,TN_Precio_Unitario")] TBL_Producto tBL_Producto)
		{
		...
		[HttpPost]
		[ValidateAntiForgeryToken]
		public ActionResult Edit([Bind(Include = "TN_IdProducto,TC_Nombre,TC_Descripcion,TN_Cantidad_Inventario,TN_Precio_Unitario")] TBL_Producto tBL_Producto)
		{
			if (ModelState.IsValid)
			{
		...
		public ActionResult ExportToPdf(string searchText, int? page)
		{
			...
			headerCell = new PdfPCell(new Phrase("Cantidad existente en inventario", headerFont));
			headerCell.BackgroundColor = headerBackgroundColor;
			headerCell.HorizontalAlignment = Element.ALIGN_CENTER;
			pdfTable.AddCell(headerCell);

			headerCell = new PdfPCell(new Phrase("Precio unitario (en colones)", headerFont));
			headerCell.BackgroundColor = headerBackgroundColor;
			headerCell.HorizontalAlignment = Element.ALIGN_CENTER;
			pdfTable.AddCell(headerCell);
			...
			foreach (var item in pagedActividad)
			{
				...
				pdfTable.AddCell(item.TN_Cantidad_Inventario.ToString());
				pdfTable.AddCell(item.TN_Precio_Unitario.ToString());
		...
		public ActionResult ExportToExcel(string searchText)
		{
			...
			using (var package = new ExcelPackage())
			{
				...
				worksheet.Cells[1, 3].Value = "Cantidad existente en inventario";
				...
				for (int i = 0; i < data.Count; i++)
				{
					...
					worksheet.Cells[i + 2, 3].Value = data[i].TN_Cantidad_Inventario.ToString();
					worksheet.Cells[i + 2, 4].Value = "₡" + data[i].TN_Precio_Unitario.ToString();
